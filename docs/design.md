...
definition, and four tests hold that absolutely. Since section 4 it sees less than that:
only the part of the layer the dashboard being worked on is scoped to.
...
- Not a production system. It is built against a synthetic oil-and-gas warehouse.
...
natural language
 ...
 [ LLM ]  ← sees: semantic layer YAML only (no data),
      │          filtered to this dashboard's schema (§4)
      │
      ▼
 semantic query JSON
      │
...
2. **Query** (interactive): natural language + the dashboard's slice of the layer →
   semantic query JSON + Vega-Lite spec. A question whose subject is not in that slice is
   refused deterministically, before this step runs at all (section 8).
...
An entity's schema is the prefix of the `table:` it already declares — `ddh` for
`ddh.fct_production_daily`, `dm_planning` for `dm_planning.field_targets_monthly`.
**Derived, never declared twice.** A `schema:` key beside `table:` would be the same fact
written in two places, free to disagree, with nothing in the loader positioned to notice.

Demand-driven coverage applies per schema as much as overall. A real `dm` database has
tens of schemas, most of which will have no entities modelled in them for a long time.
That is the ordinary state of the system, not a gap to be closed before it is useful.

### One dashboard, one schema

Each dashboard names a schema and its questions are written against that schema's
entities alone. The alternative — every entity in every schema, always — makes the menu
longer without making any single dashboard more useful, and a longer menu is a wider
target for the wrong choice.

- **Writing is scoped. Rendering is not.** Choosing an entity, building the prompt and
  suggesting example questions all use the filtered layer. Drawing a card that already
  exists uses the whole one. A card built before a dashboard was switched keeps its
  query, its chart and its place, because its query is already frozen and drawing it is
  not asking anything.
- **Switching is not destructive**, and this is the property the feature rests on. Cards
  from the previous schema stay live, refreshable and exportable. What changes is only
  what the *next* question may be about.
- **Stored on the board** as `schema_name`. Nullable in the column and never null in
  practice: creation always supplies one, and a startup routine fills legacy rows from
  the board's own cards. The migration runner is raw SQL and cannot read the layer, so it
  cannot know which schema an existing board's cards belong to; that question is only
  answerable where the layer is loaded.
- **A new dashboard inherits** the schema of the one being looked at, and a duplicate
  copies its source's. Making a second dashboard about the same subject should not ask
  again, and a copy whose cards all point somewhere its schema excludes would be
  orphaned on arrival.
- **A schema with nothing modelled in it cannot be chosen** — refused at the API, and the
  picker offers no way to switch to it. It remains browsable, because looking inside a
  schema is how somebody finds out it is empty. A dashboard pointed at one could ask
  nothing at all, and saying so once beats failing every question afterwards.

Browsing and choosing are different acts. The picker lists the **warehouse catalogue**,
not the layer — every table in every readable schema, each marked answerable or not, and
the unmodelled ones carry the honest reason: nobody has defined what their columns mean.
A list showing only modelled tables would quietly redefine the warehouse as the subset
somebody has got around to describing. The catalogue is read through the warehouse
credential, and `information_schema` shows a role only what it may access, so a schema
the app could not read cannot appear in a picker that would then fail on it.
...
A **board** carries the little that is genuinely per-dashboard:

```
board
├── id, title, position
├── schema_name       (text)   ← which schema its questions are written against
├── layout_mode       (enum: free | auto_pack)
└── revision          (bigint) ← what a frozen plan is checked against
```
...
### Questions the dashboard is not scoped to

Narrowing the menu is not free, and the direction of the cost is the opposite of the
intuition. A model shown only the planning mart and asked about oil production has no
right answer available — and a model with no right answer available does not stop. It
picks the closest wrong one. **Scoping makes the confident-wrong-chart failure more
likely, not less**, unless something catches it before the model is asked.

So the question's subject is matched against the *whole* layer first. If every measure it
names lives outside the dashboard's schema, the card refuses and says where it does live.
No API call is spent and the model never gets the chance to substitute.

**Judged on measures, never dimensions.** `region` means something in operations and in
planning both, so a question mentioning it proves nothing about where it belongs; any
rule that counted dimensions would wave "oil production by region" straight through on a
planning dashboard. A measure is what a question is *about*; a dimension is only how it
is cut.

A question whose subject is nowhere at all is left alone. Unanswerable is the ordinary
refusal path's business, and naming some other schema for it would be a lie with a schema
name in it.

The refusal is an exchange like any other (section 9), so it can be replied to rather
than merely read.
...
- **What the card last said is stored on the card, and can be replied to.** Without that
  the card asks "oil or gas?", the person types "oil", and the next request arrives as
  one word attached to nothing. The card carries the exchange — what it said, the
  *original* request, which is what has to be rebuilt, and whether it was a question or a
  refusal — and clears it once resolved so it cannot colour a later, unrelated question.
  An edit the assistant makes fills the same field the edit box does.
- **A refusal is half of an exchange too.** "The layer has no per-region ranking" is a
  sentence somebody answers with "then rank them overall", and that answer means nothing
  on its own. What differs is who decides it is a reply: a question is owed one, so
  whatever is typed next is read as the answer, while a refusal ended the exchange and
  becomes context only when the asker says they are replying to it (`reply` on `/ask`,
  always stated by the browser). Answering a question also forbids asking again; replying
  to a refusal is a fresh request and keeps the full ambiguity guard.
- **The clarifying question itself is not taken on trust.** Asked what was ambiguous, a
  model was observed filling the question field with a verbatim copy of what the person
  had typed — the card showed somebody their own sentence and waited. The ambiguous term
  and its candidates are structural and survive that, so the question put to the reader is
  rebuilt from them whenever what came back asks nothing. Same principle as the
  restatement: the sentence a card stakes its usefulness on is computed, not asserted.
...
|---|---|---|---|
| Query path (`/ask`, refinement) | the dashboard's schema only | **never** | never |
| Chart compilation | n/a | yes, but no model is involved | n/a |
| Assistant | the dashboard's schema only | only with both gates open | last N turns, as prose |

"The dashboard's schema only" is section 4's scope rule, and it applies to the assistant
exactly as it applies to a card: its prompt is built from the filtered layer, the queries
it runs and the cards it generates are written against that schema, and a question whose
subject lives elsewhere is refused by name before a model is asked.
...
A card left over from a previous schema is the one case where something on screen is
deliberately not readable. It appears in the context by title and schema **with its rows
withheld**, and the assistant is told to say which schema it belongs to rather than
answer about its numbers. Hiding it outright was the alternative and it is worse: the
card is visible, correct, and right in front of the person, and an assistant that denied
its existence would be plainly, checkably wrong. Editing one is refused for the same
reason — rewriting a query is asking a new question, and a new question belongs to the
schema the dashboard is on now.
...
**Not built.** Steps 2 and 3 of the build order were skipped and step 4 — hand-write the
layer first — was done instead and never revisited. `backend/app/bootstrap.py` seeds the
demo warehouse and has nothing to do with the layer; every entity in
`layer/definitions/` was typed by a person. The section is kept because the design is
the one to build if it is built, and because "how does it know what the columns mean" is
answered today by "somebody wrote it down", which is worth stating plainly rather than
leaving to be discovered.

Where it would matter most is exactly section 4's problem: a `dm` database with tens of
schemas needs a first draft per schema far more than a four-entity demo does.
...
20. Per-dashboard schema scope, the catalogue picker, and the cross-schema guard.

Steps 1–7 are the spine; if you stop anywhere, stop after 12.

Step 20 arrived late and belongs early. Scoping is cheap when the layer holds four
entities in one schema and increasingly awkward afterwards, because every call site that
had reached for "the layer" has to start naming which part of it. It was survivable here
only because `render`, `ask` and `validate_query` already took a layer as an argument and
merely happened to be handed the same global one every time.
