CREATE TABLE app.board (
    ...
    layout_mode text       NOT NULL DEFAULT 'free'
                       CONSTRAINT board_layout_mode_check
                       CHECK (layout_mode IN ('free', 'auto_pack')),
    -- The warehouse schema this dashboard asks its questions of.
    -- Nullable in the column and never null in practice: create_board
    -- always supplies one, and app.bootstrap fills any legacy row from the
    -- board's own cards. Left nullable so a fresh install and a migrated
    -- one have the same shape -- the migration cannot set NOT NULL before
    -- the backfill it depends on has had a chance to run.
    schema_name text,
...
CREATE TABLE app.card (
    ...
    -- Existing cards are already deliberately sized. New blank cards opt in
    -- explicitly at creation time so an upgrade never moves old dashboards.
    auto_size_pending boolean NOT NULL DEFAULT false,
    cache          jsonb,
