...
export const chatApi = {
  ...
  /** Turns a result in the transcript into a card on a dashboard. The
   *  only way anything the chat ran becomes durable. */
  keepTransient: (resultId: string, boardId: string, title?: string) =>
    request<{ id: string }>(`/chat/transient/${resultId}/keep`, {
      method: "POST",
      body: JSON.stringify({ board_id: boardId, title: title ?? "" }),
    }),
