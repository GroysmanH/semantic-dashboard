...
import type { ChartSnapshot } from "./chart";
...
function visual(card: Card): boolean {
  const render = card.render;
  return (render?.state ?? card.state) === "ready"
    && Boolean(render?.vega_spec);
}

function names(titles: string[]): string {
  if (titles.length < 2) return titles[0] ?? "an untitled card";
...
export async function boardPng(title: string, entries: ExportCard[]): Promise<Blob> {
  const drawable = entries.filter((entry) => visual(entry.card));
  if (!drawable.length) throw new Error("Nothing on this dashboard can be exported yet.");

  const settled = await Promise.allSettled(drawable.map(async (entry) => {
    if (!entry.snapshot) throw new Error("Chart snapshot is unavailable.");
    return {
      card: entry.card,
      image: await loadImage(await entry.snapshot()),
    };
  }));
  const failed = settled.flatMap((result, index) => (
    result.status === "rejected"
      ? [drawable[index].card.title || "Untitled"]
      : []
  ));
  if (failed.length) {
    throw new Error(`Could not export ${names(failed)}. No dashboard image was downloaded.`);
  }
  const images = settled.map((result) => (
    result as PromiseFulfilledResult<{ card: Card; image: HTMLImageElement }>
  ).value);
