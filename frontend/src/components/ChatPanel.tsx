...
export default function ChatPanel({
  ...
  onEditCard: (boardId: string, cardId: string) => void;
  ...
  // Results already promoted to a card, so the button says so rather than
  // offering to make a second copy of the same chart.
  ...
  const send = async (
    ...
    if (!threadId || !targetBoardId) return;
    setBusy(true);
    setError(null);
    try {
      const response = await chatApi.sendTurn(threadId, {
        active_board_id: targetBoardId,
        question,
        provider,
        hard,
        share_visible_data: shareVisibleData,
        selected_card_id: targetCardId,
      });
      const thread = await chatApi.getThread(threadId);
      setMessages(thread.messages ?? []);
  ...
  const keep = async (messageId: string, resultId: string) => {
    if (!activeBoardId) return;
    setBusy(true);
    setError(null);
    try {
      await chatApi.keepTransient(resultId, activeBoardId);
      setKept((prev) => ({ ...prev, [messageId]: true }));
      await onApplied(activeBoardId);
    } catch (e) {
      setError(e instanceof Error ? e.message : String(e));
    } finally {
      setBusy(false);
        ...
        {messages.map((message) => (
          <div key={message.id}>
            <ChatMessage
              ...
              onRetry={(text, cardId, boardId) =>
                void send(text, cardId, boardId)}
            />
            {results[message.id] && (
              <ChatResult
                result={results[message.id]}
                sqlOpen={openSql === message.id}
                kept={kept[message.id] === true}
                boardTitle={activeBoardTitle || "this dashboard"}
                onSqlToggle={() =>
                  setOpenSql(openSql === message.id ? null : message.id)}
                onRerun={() => void rerun(message.id, results[message.id].id)}
                onKeep={() => void keep(message.id, results[message.id].id)}
                busy={busy}
              />
            )}
          </div>
