...
import VegaChart, { prepareChartSpec } from "./VegaChart";
...
describe("VegaChart presentation sizing", () => {
  ...
  it("prepares the same self-contained light-theme spec used by visible and export views", () => {
    const rows = [{ region: "Atyrau", oil: 12 }];
    const config = {
      background: "transparent",
      axis: { labelColor: "#5a6c80" },
      range: { category: ["#00539b"] },
    };
    const source = {
      layer: [
        { data: { name: "outline", format: { type: "topojson", feature: "KAZ_adm1" } }, mark: "geoshape" },
        { data: { name: "table" }, mark: "circle" },
      ],
    };

    const prepared = prepareChartSpec(source, rows, { width: 760, height: 320 }, config);
    ...
    expect(prepared).toMatchObject({
      ...
      config,
      layer: [
        { data: { values: expect.any(Object), format: { type: "topojson", feature: "KAZ_adm1" } } },
        { data: { values: rows } },
      ],
    });
    expect(source.layer[1].data).toEqual({ name: "table" });
  });

  it("prepares a deterministic no-rows visual through the shared renderer", () => {
    const prepared = prepareChartSpec(
      { data: { name: "table" }, mark: "bar" },
      [],
