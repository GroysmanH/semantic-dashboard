...
export default function ChatMessage({
      ...
      <li className="chat-turn chat-user">
        ...
        {message.active_board_title && (
          <span className="eyebrow">
            Dashboard: {message.active_board_title}
          </span>
        )}
      </li>
    );
  ...
  const recoveryBoardId = recovery?.target_board_id ?? message.active_board_id;
  ...
  return (
    <li className={`chat-turn chat-assistant ${kind}`}>
      ...
      {recovery && (
        <div className="plan-actions">
          {recovery.retryable && recovery.retry_text && (
            <button
              type="button"
              disabled={undoing}
              onClick={() => onRetry(
          ...
          {recovery.target_card_id && recoveryBoardId && (
            <button
              type="button"
              disabled={undoing}
              onClick={() => onEditCard(
                recoveryBoardId, recovery.target_card_id!,
              )}
            >
              Edit card
            </button>
      ...
      {message.data_exposed && (
        <span className="eyebrow chat-exposed">
          Used the numbers shown on this dashboard.
        </span>
      )}
    </li>
  );
