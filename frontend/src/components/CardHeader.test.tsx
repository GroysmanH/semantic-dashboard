...
function setup(cardRender: Render = READY_RENDER) {
  const props = {
    title: "Oil by month",
...
describe("CardHeader exports", () => {
  ...
  it("keeps duplication and removal in the card actions menu", async () => {
    const user = userEvent.setup();
    setup();

    await user.click(screen.getByRole("button", { name: "Card actions" }));
    const actions = screen.getByRole("menu", { name: "Card actions" });
    expect(within(actions).getAllByRole("menuitem")).toHaveLength(2);
    expect(within(actions).getByRole("menuitem", { name: "Duplicate card" })).toBeVisible();
  ...
  it("keeps a data-quality warning visible beside the card's trust details", () => {
    const renderWithWarning = {
      ...READY_RENDER,
      quality_warnings: ["Well downtime: The source unit for well downtime is not documented."],
    } as Render & { quality_warnings: string[] };

    setup(renderWithWarning);
  ...
  it("renders a repeated data-quality warning once", () => {
    const warning = "Well downtime: The source unit is not documented.";

    setup({
      ...READY_RENDER,
      quality_warnings: [warning, warning],
    });
