...
function setup(over: Partial<Parameters<typeof ChatPanel>[0]> = {}) {
  const props = {
    ...
    boards: [{
      ...
      layout_mode: "free" as const, schema_name: "ddh", revision: 1,
    }],
...
describe("ChatPanel", () => {
  ...
  it("keeps Try again usable after a recoverable refusal is reloaded", async () => {
    const user = userEvent.setup();
    vi.mocked(chatApi.getThread).mockResolvedValue({
      id: "t1",
      messages: [{
        id: "m-format", role: "assistant", action: "refuse",
        refusal: "I couldn’t prepare that change safely. Nothing changed.",
        failure_code: "model_format_invalid",
        recovery: {
          retryable: true,
          retry_text: "rename this dashboard",
          target_card_id: null,
        },
    ...
    vi.mocked(chatApi.sendTurn).mockResolvedValue({
      message: { id: "m2", role: "assistant", action: "answer", say: "ok",
                 created_at: "now" },
    } as never);
    setup();
    ...
    expect(chatApi.sendTurn).toHaveBeenCalledWith("t1", expect.objectContaining({
      question: "rename this dashboard",
      provider: "anthropic",
      active_board_id: "b1",
    }));
  ...
  it("retries on the refusal's dashboard after another dashboard is opened", async () => {
    const user = userEvent.setup();
    vi.mocked(chatApi.getThread).mockResolvedValue({
      id: "t1",
      messages: [{
        ...
        recovery: {
          retryable: true,
          retry_text: "make this a line chart",
          target_card_id: "c1",
          target_board_id: "b1",
        },
        active_board_id: "b1", active_board_title: "Operations",
    ...
    expect(chatApi.sendTurn).toHaveBeenCalledWith("t1", expect.objectContaining({
      active_board_id: "b1",
      selected_card_id: "c1",
      question: "make this a line chart",
    }));
  });

  it("offers Edit card only when persisted recovery names one", async () => {
    const user = userEvent.setup();
    vi.mocked(chatApi.getThread).mockResolvedValue({
      id: "t1",
      messages: [{
    ...
    expect(props.onEditCard).toHaveBeenCalledWith("b1", "c1");
    expect(props.onNavigate).not.toHaveBeenCalled();
  });

  it("recovers an edit on its target dashboard instead of the active dashboard", async () => {
    const user = userEvent.setup();
    vi.mocked(chatApi.getThread).mockResolvedValue({
      id: "t1",
      messages: [{
        ...
        recovery: {
          retryable: true,
          retry_text: "show this by month",
          target_card_id: "card-on-b",
          target_board_id: "board-b",
        },
        active_board_id: "board-a", active_board_title: "Dashboard A",
        created_at: "now",
      }],
    } as never);
    vi.mocked(chatApi.sendTurn).mockResolvedValue({
    ...
    const props = setup({
      ...
      activeBoardTitle: "Dashboard A",
      boards: [
        { id: "board-a", title: "Dashboard A", position: 0,
          layout_mode: "free", schema_name: "ddh", revision: 1 },
        { id: "board-b", title: "Dashboard B", position: 1,
          layout_mode: "free", schema_name: "ddh", revision: 1 },
      ],
    });

    await user.click(await screen.findByRole("button", { name: "Edit card" }));
    expect(props.onEditCard).toHaveBeenCalledWith("board-b", "card-on-b");

    await user.click(screen.getByRole("button", { name: "Try again" }));
    expect(chatApi.sendTurn).toHaveBeenCalledWith("t1", expect.objectContaining({
      active_board_id: "board-b",
      selected_card_id: "card-on-b",
      question: "show this by month",
    }));
  });

  it("uses common-language dashboard and data metadata", async () => {
    vi.mocked(chatApi.getThread).mockResolvedValue({
      id: "t1",
      messages: [
        { id: "u1", role: "user", action: "ask", say: "Which leads?",
          active_board_title: "Operations", created_at: "now" },
        { id: "a1", role: "assistant", action: "answer", say: "Atyrau.",
          data_exposed: true, created_at: "now" },
      ],
    } as never);
    setup();

    expect(await screen.findByText("Dashboard: Operations")).toBeVisible();
    expect(screen.getByText("Used the numbers shown on this dashboard."))
      .toBeVisible();
    expect(screen.queryByText("read the visible rows")).not.toBeInTheDocument();
  });
  ...
  it("keeps a chat result as a card, on the dashboard in front of you",
     async () => {
    // Ask a question, get a chart, and until now have no way to keep it.
    const user = userEvent.setup();
    vi.mocked(chatApi.sendTurn).mockResolvedValue({
      message: { id: "m1", role: "assistant", action: "run_query",
                 say: "Here it is.", created_at: "x" },
      transient_result: { id: "r1", restatement: "Sum of oil, by region.",
                          rows: [{ region: "Atyrau", oil: 1 }], row_count: 1,
                          compiled_sql: "SELECT 1" },
    } as never);
    vi.mocked(chatApi.keepTransient).mockResolvedValue({ id: "c1" } as never);
    // The panel re-reads the thread after a turn, so the transcript it
    // renders is the stored one rather than the response.
    vi.mocked(chatApi.getThread).mockResolvedValue({
      id: "t1",
      messages: [{ id: "m1", role: "assistant", action: "run_query",
                   say: "Here it is.", created_at: "x" }],
    } as never);
    const props = setup();

    await user.type(screen.getByLabelText("Ask about this dashboard…"), "oil");
    await user.click(screen.getByRole("button", { name: "Send" }));
    await user.click(
      await screen.findByRole("button", { name: /Keep it on Operations/ }));

    expect(chatApi.keepTransient).toHaveBeenCalledWith("r1", "b1");
    // The board has a new card on it, so the grid has to be reloaded.
    await waitFor(() => expect(props.onApplied).toHaveBeenCalledWith("b1"));
    expect(await screen.findByText(/Kept on Operations/)).toBeVisible();
  });
});
