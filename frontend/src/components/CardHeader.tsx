...
export default function CardHeader({
  ...
  const qualityWarnings = [...new Set(render?.quality_warnings ?? [])];
  ...
  return (
    <div className="card-head">
          ...
          {menuOpen && (
            <div className="card-menu-popover" role="menu" aria-label="Card actions">
              {onDuplicate && (
                ...
                >
                  {duplicating ? "Duplicating…" : "Duplicate card"}
                </button>
              )}
              <button
                className="remove-card-action"
                type="button"
                role="menuitem"
                disabled={busy}
                onClick={() => {
      ...
      {hasValidResult && qualityWarnings.map((warning, index) => (
        <p className="quality-warning" role="alert" key={`${index}:${warning}`}>{warning}</p>
      ))}
    </div>
  );
}
