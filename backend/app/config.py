...
class Settings(BaseSettings):
    ...
    max_rows: int = 10_000
    # The schema a board falls back to when nothing else decides: a brand
    # new install, or a board whose cards say nothing because it has none.
    # Not derived from the layer, because "whichever entity happened to
    # sort first" is not a decision anybody made.
    default_schema: str = "ddh"

    # A question is a sentence; a scan of a fact table is not. Nothing in
    # this app bounds how long a compiled query may run, and against a
    # real warehouse the cost of that lands on somebody else's database.
    # Set on the session rather than per statement so it covers every
    # query the pool ever issues, including ones added later.
    warehouse_statement_timeout: str = "30s"

    # Which schemas this app may show or be pointed at. Empty means every
    # schema the credential can read, which is the right default for a
    # warehouse built for the app and the wrong one for a warehouse the
    # app was let into: the first real credential here could read payroll,
    # personal data and medical records across four HR schemas it had no
    # business with. Narrowing the grant is the proper fix and belongs to
    # whoever owns the database; this is what the app can do meanwhile,
    # and it is worth doing because "nobody modelled it" is not access
    # control -- it is one YAML file away from being queryable.
    visible_schemas: str = ""

    @property
    def schema_allowlist(self) -> set[str]:
        return {s.strip() for s in self.visible_schemas.split(",") if s.strip()}
