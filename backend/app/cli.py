...
from .layer.loader import load_layer
from .layer.models import LayerError
...
def validate_all() -> int:
    """Validate shipped definitions without opening any external connection."""
    try:
        layer = load_layer(settings.layer_dir)
        migrations = discover_migrations(settings.migration_dir)
    except (LayerError, MigrationError, OSError, yaml.YAMLError):
        print("validation failed", file=sys.stderr)
        return 1
    print(
        f"validated {len(layer)} semantic definition(s) and "
...
def main() -> int:
    ...
    sub.add_parser(
        "validate-all",
        help="validate semantic definitions and migration files offline",
    )
