...
MAX_CATEGORIES = 30            # beyond this labels become an index, not a chart
...
def _labels(compiled: CompiledQuery) -> dict[str, str]:
    ...
    for ref in compiled.query.measures:
        ...
        unit = compiled.output_units.get(ref.output_name)
        if unit:
            base += " (%)" if unit == "percent" else f" ({unit})"
...
def _number_format(compiled: CompiledQuery, column: str) -> dict[str, Any]:
    ...
    if compiled.output_units.get(column) == "percent":
        return {"axis": {"format": ".1f"}}
    if compiled.output_units.get(column) == "tonnes":
        return {"axis": {"format": ",.0f"}}
...
def _tooltip_from_encoding(encoding: dict[str, Any]) -> dict[str, Any]:
    ...
    return _tooltip_enc(
        ...
        number_format=axis_format if isinstance(axis_format, str) else None)
...
def _measure_tooltip(compiled: CompiledQuery, field_: str,
    ...
    return _tooltip_enc(
        ...
        number_format=(".1%" if _display_format(compiled, field_) == ".1%"
                       else _display_format(compiled, field_) if compiled.output_units.get(field_)
                       else ",.2f"),
    )
    ...
    *,
    ...
    if shared:
        ...
        if series_field is None:
            ...
            number_format = (
                y["axis"].get("format") if isinstance(y.get("axis"), dict)
                else None
            )
            if fold_field and len(generated) > 1:
                for series_index, value in enumerate(values):
                    ...
                    tooltip.append(_tooltip_enc(
                        output, "quantitative",
                        f"{y.get('title', 'Value')} — {titles.get(value, value)}",
...
def _top_n(rows: list[dict[str, Any]], dim: str, measure: str,
           limit: int = MAX_CATEGORIES) -> tuple[list[dict[str, Any]], str]:
    """The largest `limit` categories, and a sentence saying so.

    Dropped rather than collapsed into "Other". A pie is a claim about a
    whole, so an "Other" slice keeps it honest; a ranked bar chart is a
    claim about order, and a bar labelled "Other" reads as a category that
    outranks real ones. Showing the top of the ranking and saying how much
    was left off is the truthful shape for this chart.

    Refusing outright, which is what happened before, is worse than either.
    "well has 100 categories" is a real answer to a real question, and a
    card that draws nothing at all leaves the person no way to see it --
    the numbers are right there in the SQL panel and the row count.
    """
    if len(rows) <= limit:
        return rows, ""
    ordered = sorted(rows, key=lambda r: float(r.get(measure) or 0),
                     reverse=True)
    note = f"top {limit} of {len(rows)} shown, ranked by value"
...
def build_spec(compiled: CompiledQuery, rows: list[dict[str, Any]] | None = None,
    ...
    if n_dims == 1:
        ...
        if n_meas == 1:
            ...
            if dim_kind == "nominal":
                # Too many bars to label used to be a refusal. It is a
                # ranking, so the honest shape is the top of it with the
                # truncation stated -- the same trade the pie already
                # makes, and the note reaches the restatement the same way.
                chart_rows, note = _top_n(rows, dim, measure)
                spec.update(_horizontal_bar(
                    compiled, chart_rows, dim, measure, labels, mark=mark))
                return done(ChartResult(
                    spec, _mark(mark),
                    chart_rows=chart_rows if note else None, note=note))
