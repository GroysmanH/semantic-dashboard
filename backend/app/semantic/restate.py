...
def _measure_phrase(entity: Entity, ref: MeasureRef) -> str:
    ...
    unit = output_unit(entity, ref)
    if unit:
        label += " (%)" if unit == "percent" else f" ({unit})"
...
def restate(q: SemanticQuery, entity: Entity, row_count: int | None = None,
    ...
    # Said in the sentence, not only in a row count: "100 rows" reads as
    # the answer's size, where "more rows exist" reads as its edge.
    #
    # The two clauses compose rather than stack. A chart that collapsed its
    # own tail already says "top 30 of 100 shown"; following that with
    # "top 100, more exist" gives the reader two different top-somethings
    # to reconcile, when what is true is one sentence: thirty of a hundred
    # shown, out of more than a hundred.
    if truncated and note:
        sentence += f" — {note}, out of more than {limit} rows"
    elif truncated:
        sentence += f" — showing the top {limit}; more rows exist"
    elif note:
        sentence += f" — {note}"
