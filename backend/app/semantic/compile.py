...
def _window_expr(ref: MeasureRef, temporal: list[str],
    ...
    if ref.transform == "rank":
        scope = sql.SQL("PARTITION BY {} ").format(
            sql.SQL(", ").join(sql.Identifier(t) for t in temporal)) if temporal else sql.SQL("")
        return sql.SQL("RANK() OVER ({}ORDER BY {} DESC NULLS LAST)").format(
            scope, col
        ), []
    raise ValueError(f"unhandled transform {ref.transform!r}")   # pragma: no cover


def _inline(statement: str, params: list[Any]) -> str:
    """The same statement with its values written in, for reading only.

    What executes is always the parameterised form: values reach Postgres
    beside the query, never inside it, which is why nothing a person or a
    model types can change what a query does. That is also why the SQL a
    card shows cannot be pasted into a database client -- `%s` is not SQL,
    and the client stops at the first one.

    So the panel gets a second rendering, quoted by psycopg rather than by
    hand. It is display output and nothing runs it. If it ever became the
    thing that executed, the injection guarantee above would be gone.
    """
    out: list[str] = []
    rest = statement
    for value in params:
        head, sep, rest = rest.partition("%s")
        if not sep:                      # more values than slots: don't guess
            return statement
        out.append(head)
        out.append(sql.Literal(value).as_string(None))
    return "".join(out) + rest


def _uses_text_collation(
    field: str, *, kinds: dict[str, ColumnKind], entity: Entity
) -> bool:
    dimension = entity.dimensions.get(field)
    return (
        kinds.get(field) == "nominal"
...
def _order_part(
    ...
    *,
    ...
    """One deterministic output order term.

    Text dimensions use PostgreSQL's bytewise C collation so the runtime
    validator can reproduce the same order independently of database locale.
    Numeric measures and temporal outputs retain their native ordering.
    """
    expression: sql.Composable = sql.Identifier(field)
    if _uses_text_collation(field, kinds=kinds, entity=entity):
        expression = sql.SQL("{} COLLATE {}").format(
            expression, sql.Identifier("C")
        )
    return sql.SQL("{} {} NULLS LAST").format(
        expression, sql.SQL(direction.upper())
    )
...
def compile_query(q: SemanticQuery, layer: Layer) -> CompiledQuery:
    ...
    for ref in q.measures:
        ...
        target = entity.measure(ref.name)
        output_units[ref.output_name] = output_unit(entity, ref)
        if target is not None and target.quality_warning:
            quality_warnings[ref.output_name] = target.quality_warning
    ...
    # Filters may pull in a join no dimension needed. The entity's own
    # constraints go first and are not optional: they say what the
    # entity is, so no query -- and no model -- can ask past them.
    where: list[sql.Composable] = []
    where_params: list[Any] = []
    for f in (*entity.filters, *q.filters):
    ...
    order_fields = [ob.field for ob in q.order_by]
    named = {ob.field for ob in q.order_by}

    # An unordered LIMIT is not a sample, it is a lottery. On a
    # distributed warehouse the rows that arrive first are whichever
    # segment answered first, so the same question asked twice returned a
    # different hundred of the same hundred-and-twenty-five companies --
    # measured, not supposed. Ordering by the leading measure turns an
    # arbitrary hundred into a top hundred, which is at least a thing
    # somebody meant to ask for.
    if not parts and q.dimensions:
        parts.append(_order_part(
            q.measures[0].output_name,
            "desc",
            kinds=kinds,
            entity=entity,
        ))
        order_fields.append(q.measures[0].output_name)
        named.add(q.measures[0].output_name)

    # And a tiebreaker under whatever was asked for, because ordering by a
    # measure alone is only deterministic until two rows tie.
    if parts:
        for column in columns:
            if kinds.get(column) != "quantitative" and column not in named:
                parts.append(_order_part(
                    column,
                    "asc",
                    kinds=kinds,
        ...
        # A bare SELECT-list alias is visible to ORDER BY, but an alias
        # nested inside COLLATE is not. Materialise the outputs once so the
        # explicit collation always applies to the compiled alias.
        if any(
            _uses_text_collation(field, kinds=kinds, entity=entity)
            for field in order_fields
        ):
            stmt = (
                sql.SQL("SELECT * FROM (")
                + stmt
                + sql.SQL(") AS {} ").format(sql.Identifier("ordered_result"))
            )
        stmt += sql.SQL(" ORDER BY ") + sql.SQL(", ").join(parts)

    # One more row than anybody wants to see. If it comes back, there was
    # more, and the card can say so instead of presenting a fraction of an
    # answer as the answer.
    row_limit = min(q.limit, MAX_ROWS)
    stmt += sql.SQL(" LIMIT %s")
    params.append(row_limit + 1)

    return CompiledQuery(
        sql=stmt.as_string(None),
        sql_readable=_inline(stmt.as_string(None), params),
        params=params,
        row_limit=row_limit,
        columns=columns,
        column_kinds=kinds,
