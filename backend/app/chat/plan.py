...
from ..layer.scope import layer_for, schema_of
from ..llm.client import LLMClient
from ..llm.query_step import ask as ask_model
from ..layout import canonical_layouts, visualization_size
...
class ModelFormatInvalid(RuntimeError):
    """A nested query missed its schema twice; carry only its safe target."""

    def __init__(self, target_card_id: uuid.UUID,
                 target_board_id: uuid.UUID) -> None:
        super().__init__("nested query format invalid")
...
def _move_operations(board: dict, cards: list[dict],
                     final: dict[str, dict], *, skip=()) -> list[PlanOperation]:
    skipped = {str(card_id) for card_id in skip}
    operations: list[PlanOperation] = []
    for card in cards:
        card_id = str(card["id"])
        if card_id in skipped or card_id not in final:
            continue
...
def _new_cards(action, *, board: dict, cards: list[dict]) -> Resolved:
    placed = _card_previews(action.cards,
                            [c.get("layout") or {} for c in cards])
    planned_ids = [uuid.uuid4() for _ in placed]
    current = {str(card["id"]): _clamp(card.get("layout") or {})
               for card in cards}
    desired = {
        **current,
        **{str(card_id): preview.layout.model_dump()
           for card_id, preview in zip(planned_ids, placed)},
    }
    layouts = canonical_layouts(
        desired, auto_pack=board["layout_mode"] == "auto_pack",
    )
    previews = [
        PlanCardPreview(
    ...
    return Resolved(
        ...
        resolved={
            ...
            "cards": [
                ...
                for card_id, p, r in zip(planned_ids, previews, action.cards)
            ],
            "layouts": layouts,
        },
        basis={},
        operations=[*create_operations,
                    *_move_operations(board, cards, layouts)],
        cards=previews,
...
def _new_dashboard(action, *, board: dict) -> Resolved:
    ...
    planned_ids = [uuid.uuid4() for _ in previews]
    layouts = {
        str(card_id): preview.layout.model_dump()
        for card_id, preview in zip(planned_ids, previews)
    }
    return Resolved(
        ...
        resolved={
            ...
            "cards": [
                {**p.model_dump(mode="json"), "card_id": str(card_id),
                 "chart_hint": r.chart_hint}
                for card_id, p, r in zip(planned_ids, previews, action.cards)
            ],
            "layouts": layouts,
        ...
        operations=[
            ...
            *[
                PlanOperation(kind="create_card",
                              summary=f"Add “{p.title}” with initial position "
                                      f"{_where(p.layout.model_dump())}",
...
def _edit_card(intent, *, boards: list[dict], client: LLMClient,
    ...
    # A card whose entity is outside its dashboard's schema is one built
    # before the switch. It renders, it exports, it is undisturbed -- but
    # rewriting its query is asking a new question, and a new question
    # belongs to the schema the dashboard is on now.
    scope = board["schema_name"] or settings.default_schema
    entity = LAYER.get(card["semantic_query"].get("entity") or "")
    if entity is not None and schema_of(entity) != scope:
        raise PlanRefused(
            f"That card reads the {schema_of(entity)} schema and this "
            f"dashboard is set to {scope}. Switch the dashboard's schema to "
            f"change it.")

    current = SemanticQuery.model_validate(card["semantic_query"])
    outcome = ask_model(intent.instruction, layer_for(LAYER, scope), client,
                        synonyms=SYNONYMS,
                        current=current, request_id=request_id,
                        stage="edit_card")
    if outcome.failure_code == "model_format_invalid":
        raise ModelFormatInvalid(card["id"], card["board_id"])
    if outcome.refusal:
...
def _layout(action, *, board: dict, cards: list[dict]) -> Resolved:
    ...
    current = {card_id: _clamp(card.get("layout") or {})
    ...
    for change in action.changes:
        ...
        desired[str(change.card_id)] = _clamp(change.layout.model_dump())

    layouts = canonical_layouts(
        desired, auto_pack=board["layout_mode"] == "auto_pack",
    )
    operations = _move_operations(board, cards, layouts)
...
def _delete_card(action, *, boards: list[dict]) -> Resolved:
    ...
    target_ids = {str(card["id"]) for card in targets}
    layouts_by_board: dict[str, dict[str, dict]] = {}
    move_operations: list[PlanOperation] = []
    for board_id in dict.fromkeys(card["board_id"] for card in targets):
        target_board = by_board[board_id]
        remaining = [card for card in store.list_cards(board_id)
                     if str(card["id"]) not in target_ids]
        current = {str(card["id"]): _clamp(card.get("layout") or {})
                   for card in remaining}
        final = canonical_layouts(
            current,
            auto_pack=target_board["layout_mode"] == "auto_pack",
        )
        layouts_by_board[str(board_id)] = final
        move_operations.extend(_move_operations(target_board, remaining, final))

    board = by_board[targets[0]["board_id"]]
    delete_operations = [
        PlanOperation(
            kind="delete_card",
    ...
    return Resolved(
        ...
        resolved={"kind": "delete_card", "board_id": str(board["id"]),
                  ...
                  "layouts": layouts_by_board[str(board["id"])],
                  "layouts_by_board": layouts_by_board},
        basis={},
        operations=[*delete_operations, *move_operations],
        target_board_id=board["id"], target_board_title=board["title"],
    )
...
def resolve(kind: str, detail, *, board: dict, boards: list[dict],
            cards: list[dict], client: LLMClient,
            request_id: str = "") -> Resolved:
    ...
    frozen = out.resolved
    if out.kind == "layout" and out.target_board_id is not None:
        frozen = {
            **frozen,
            "expected_revision": basis[str(out.target_board_id)],
        }
    return replace(out, basis=basis, resolved=frozen)
...
def apply_immediate(resolved: dict) -> Applied:
    ...
    if kind == "layout":
        ...
        try:
            result = store.save_layouts(
                ...
                expected_revision=resolved.get("expected_revision"),
            )
        except store.BoardLayoutConflict as exc:
            raise PlanRefused(
                "That dashboard changed after this preview. Nothing changed."
            ) from exc
        if result is None:
            raise PlanRefused("That dashboard has gone.")
        after = result["layouts"]
        if after != resolved["layouts"]:
            raise PlanRefused(
                "That layout can no longer be applied exactly. Nothing changed."
            )
        affected = {card_id for card_id in before
        ...
        return Applied(summary=f"Moved {moved} card"
                       ...
                       undo={"kind": "layout", "board_id": str(board_id),
                             "was": {card_id: before[card_id]
                                     for card_id in affected},
                             "left": {card_id: after[card_id]
                                      for card_id in affected}})
    ...
    if kind == "delete_card":
        ...
        if not removed:
            raise PlanRefused("Those cards have already gone.")
        layouts_by_board = resolved.get("layouts_by_board") or {
            resolved["board_id"]: resolved.get("layouts", {})
        }
        for stored_board_id, layouts in layouts_by_board.items():
            result = store.save_layouts(
                uuid.UUID(stored_board_id), layouts, require_complete=True,
            )
            if result is None or result["layouts"] != layouts:
                raise PlanRefused(
                    "That deletion can no longer be applied exactly."
                )
...
def create_placeholders(resolved: dict) -> tuple[uuid.UUID, list[dict]]:
    ...
    for request in resolved["cards"]:
        planned_id = (uuid.UUID(request["card_id"])
                      if request.get("card_id") else None)
        card = store.create_card(
            board["id"], layout=request["layout"], card_id=planned_id,
    ...
    if resolved.get("layouts") is not None:
        result = store.save_layouts(
            board["id"], resolved["layouts"], require_complete=True,
        )
        if result is None or result["layouts"] != resolved["layouts"]:
            raise PlanRefused(
                "Those cards can no longer be placed exactly as previewed."
            )
...
def build_card(
    ...
    *,
    ...
    client: LLMClient,
    scope: str = "",
    batch_card_ids: list[uuid.UUID] | None = None,
    insertion_row: int = 0,
    conn=None,
) -> str | None:
    ...
    """
    outcome = ask_model(question, layer_for(LAYER, scope) if scope else LAYER,
                        client, synonyms=SYNONYMS)
    if outcome.refusal:
    ...
    card = store.get_card(card_id, conn=conn)
    if card is None:
    ...
    if batch_card_ids is None:
        saved = store.save_rendered_card(
            card_id, size=size, conn=conn, **fields
        )
    else:
        saved = store.save_rendered_batch_card(
            card_id,
