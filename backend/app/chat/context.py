...
def build_context(
    *,
    ...
    for card in ordered:
        ...
        # A card built before the dashboard's schema was switched is still
        # on screen and still correct, so pretending it is not there would
        # have the assistant deny something the person is looking at. It is
        # named, and its numbers are withheld: this dashboard is not the
        # place those questions get answered.
        card_schema = schema_of_card(card) if schema_of_card else ""
        if scope and card_schema and card_schema != scope:
            block.append(f"  schema: {card_schema}")
            block.append("  note: out of this dashboard's schema -- say which "
                         "schema it belongs to and that switching is needed; "
                         "do not answer questions about its numbers")
