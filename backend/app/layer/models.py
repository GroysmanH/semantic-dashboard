...
class Constraint(BaseModel):
    """A condition true of every row this entity describes.

    Real warehouses stack several facts in one table and tell them apart
    with a discriminator column -- production and delivery of the same
    oil, in the same table, distinguished by `type`. Modelled as one
    entity that is a sum over the whole table, "how much oil in August"
    counts the same barrels twice and looks entirely normal doing it.

    So the discriminator becomes part of what the entity *is*, not
    something a question has to remember to say. Two entities over one
    table, each constrained to its own slice, and neither able to see the
    other's rows: the double count stops being a mistake somebody can
    make.

    Layer-authored and never model-authored, like a join condition. The
    model cannot add, drop or read past one -- these clauses are welded
    on after whatever it asked for.
    """
    ...
    def _check(self) -> Constraint:
        if not IDENTIFIER.match(self.field):
            raise ValueError(f"not a valid identifier: {self.field!r}")
        if self.op == "in" and not isinstance(self.value, list):
            raise ValueError("op 'in' needs a list of values")
