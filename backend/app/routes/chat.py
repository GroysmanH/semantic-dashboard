...
@router.post(
    ...
    responses={
        429: {"model": ChatErrorEnvelope},
        500: {"model": ChatErrorEnvelope},
    },
)
def take_turn(thread_id: uuid.UUID, body: TurnIn,
              http_request: Request) -> ChatTurnResponse:
    ...
    try:
        ...
        return run_turn(turn_request, client=client)
        ...
        envelope = ChatErrorEnvelope(
            code="chat_rate_limited", message=str(exc),
            request_id=request_id, retryable=True,
        )
        return JSONResponse(status_code=429, content=envelope.model_dump())
    except HTTPException:
        # Expected preflight outcomes keep their exact status and detail;
        # only unexpected store failures enter the safe 500 envelope.
        raise
    except Exception as exc:  # noqa: BLE001
        logger.exception(
            "unexpected chat turn failure",
            extra={
                "request_id": request_id,
                "provider": getattr(client, "provider", body.provider or ""),
                "model": getattr(client, "model", ""),
                "stage": getattr(exc, "stage", "router"),
                "schema": getattr(exc, "schema", "ChatRouterResponse"),
...
@router.post("/transient/{result_id}/keep")
def keep_transient(result_id: uuid.UUID, body: KeepIn):
    """Promote a chat result into a card on a dashboard.

    The one path by which anything the chat ran becomes durable, and it is
    a button somebody presses. The query is re-rendered rather than the
    cached rows being copied across: a result can be minutes old, and a
    card that arrives already stale is the failure the freshness line
    exists to catch.
    """
    stored = chat_store.get_transient(result_id)
    if stored is None:
        raise HTTPException(404, "that result is no longer available")
    if store.get_board(body.board_id) is None:
        raise HTTPException(404, "no such dashboard")

    from ..render import is_persistable
    ...
    card = store.create_card(body.board_id, auto_size_pending=True)
    if card is None:
        raise HTTPException(404, "no such dashboard")
    size = (
        visualization_size(
    ...
    saved = store.save_rendered_card(
        card["id"],
        size=size,
        title=body.title or stored["title"] or r.restatement or "Untitled",
