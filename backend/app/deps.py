...
def example_questions(layer: Layer | None = None,
                      limit: int = 6) -> list[str]:
    """Drawn from the layer, so the empty card doubles as the discoverability
    mechanism for what vocabulary actually exists.

    Takes a layer rather than reading the global one, because a dashboard
    scoped to a schema needs examples from that schema. Suggesting a
    question the board would then refuse is worse than suggesting nothing.
    """
    out: list[str] = []
    for entity in (LAYER if layer is None else layer).values():
