...
class ActionItemTransitionError(RuntimeError):
    """The action item no longer has the state the caller expected."""
...
def create_thread(*, conn=None) -> dict[str, Any]:
    with _connection(conn) as active:
        ...
        purge_expired_transients(conn=active)
        return _q(
...
def transition_action_item(
    ...
    *,
    ...
    if expected not in allowed or status not in allowed:
        raise ValueError("unknown action item status")
    if status not in allowed[expected]:
        raise ValueError(
            f"illegal action item transition: {expected} -> {status}"
        )
    item = _q(
        "UPDATE app.chat_action_item SET status = %s, error = %s, "
        f"updated_at = now() WHERE id = %s AND status = %s "
        f"RETURNING {ITEM_COLUMNS}",
        (status, error, item_id, expected),
        fetch="one",
        conn=conn,
    )
    if item is None:
        raise ActionItemTransitionError(
            "action item status changed before transition"
        )
    return item
...
def save_transient(
    ...
    *,
    ...
    with _connection(conn) as active:
        # Swept on write, by this application, because there is no
        # scheduler to sweep it anywhere else. These rows hold warehouse
        # values: an expiry that only greys out the card and leaves the
        # data in the table is a retention promise the app is not keeping.
        # Every new result pays for removing the ones that have run out.
        purge_expired_transients(conn=active)
        return _q(
            f"INSERT INTO app.chat_transient_result "
            f"(id, thread_id, query, chart_hint, title, cache, expires_at) "
            f"VALUES (%s, %s, %s, %s, %s, %s, "
            f"now() + make_interval(secs => %s)) "
            f"RETURNING {TRANSIENT_COLUMNS}",
            (
