...
class _OpenAICompatible:
    ...
    def ask(self, system: str, user: str, schema: type[T]) -> T:
        ...
        try:
            ...
            # OpenAI-compatible SDKs validate inside ``parse`` and can
            # raise before returning a response. It is still a grammar
            # miss, so it belongs on the same one-retry path as validation
            # performed below on a JSON body.
            raise LLMSchemaError(_schema_reason(exc)) from exc
