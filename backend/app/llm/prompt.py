...
def build_system_prompt(layer: Layer) -> str:
    ...
    for name in sorted(layer):
        ...
        for c in e.filters:
            # Said out loud, because a number that looks low is otherwise
            # indistinguishable from a number that is wrong.
            shown = (", ".join(str(v) for v in c.value)
                     if isinstance(c.value, list) else c.value)
            out.append(f"scope: every row has {c.field} {c.op} {shown}. "
                       f"This is already applied; do not filter on it.")
