...
def test_layout_policy_migration_preserves_existing_rows_as_free_and_sized(
    ...
    with psycopg.connect(disposable_admin_dsn, autocommit=True) as conn, conn.cursor() as cur:
        cur.execute("INSERT INTO app.board (id) VALUES (%s)", (board_id,))
        cur.execute("INSERT INTO app.card (id) VALUES (%s)", (card_id,))

    run_migrations(disposable_admin_dsn, _production_migration_dir())

    with psycopg.connect(disposable_admin_dsn) as conn, conn.cursor() as cur:
        cur.execute("SELECT layout_mode FROM app.board WHERE id = %s", (board_id,))
        assert cur.fetchone() == ("free",)
        cur.execute(
            "SELECT auto_size_pending FROM app.card WHERE id = %s", (card_id,)
        )
        assert cur.fetchone() == (False,)
        with pytest.raises(psycopg.errors.CheckViolation):
            cur.execute(
                "INSERT INTO app.board (id, layout_mode) VALUES (%s, 'stacked')",
                (uuid4(),),
            )
...
def test_fresh_install_sql_and_upgrade_migration_converge_securely(
    ...
    if not init_dir.exists():
        init_dir = Path(__file__).parents[2] / "db" / "init"
    with psycopg.connect(disposable_admin_dsn, autocommit=True) as conn, conn.cursor() as cur:
        # The real file, not a hand-copied CREATE SCHEMA pair. Those went
        # stale the first time a schema was added -- the same drift this
        # test warns about for migration versions two lines below.
        cur.execute((init_dir / "01_schemas.sql").read_text(encoding="utf-8"))
        cur.execute((init_dir / "03_app.sql").read_text(encoding="utf-8"))
