...
def layout_map(board):
    return {str(card["id"]): card["layout"] for card in cards_of(board)}
...
def test_resolving_new_cards_creates_nothing(board, boards):
    ...
    assert "initial position" in out.operations[0].summary
...
def test_a_new_dashboard_has_no_board_id_until_it_exists(board, boards):
    ...
    assert [o.kind for o in out.operations] == ["create_dashboard",
                                                "create_card"]
    assert "initial position" in out.operations[1].summary
...
def test_free_layout_preview_freezes_the_complete_canonical_result(board, boards):
    first = a_card(board, title="First")
    second = a_card(board, title="Second")
    moving, displaced = sorted((first, second), key=lambda card: str(card["id"]))
    store.save_layouts(board["id"], {
        str(moving["id"]): {"x": 0, "y": 0, "w": 6, "h": 10},
        str(displaced["id"]): {"x": 6, "y": 0, "w": 6, "h": 10},
    })
    out = planner.resolve(
        "layout",
        LayoutAction(changes=[{
            "card_id": str(moving["id"]),
            "layout": {"x": 6, "y": 0, "w": 6, "h": 10},
        }]),
        board=store.get_board(board["id"]), boards=store.list_boards(),
        cards=cards_of(board), client=FakeAsk(),
    )

    frozen = out.resolved["layouts"]
    affected = {
        operation.card_id for operation in out.operations
        if operation.kind == "move_card"
    }
    assert set(frozen) == {str(first["id"]), str(second["id"])}
    assert affected == {moving["id"], displaced["id"]}

    planner.apply_immediate(out.stored())
    assert layout_map(board) == frozen
...
def test_auto_packed_chat_layout_records_the_whole_canonical_change_for_undo(
    board, boards,
):
    first = a_card(board, title="First")
    second = a_card(board, title="Second")
    store.save_layouts(board["id"], {
        str(first["id"]): {"x": 0, "y": 0, "w": 6, "h": 10},
        str(second["id"]): {"x": 0, "y": 10, "w": 6, "h": 10},
    })
    store.update_board(board["id"], layout_mode="auto_pack")
    out = planner.resolve(
        "layout",
        LayoutAction(changes=[{
            "card_id": str(first["id"]),
    ...
    applied = planner.apply_immediate(out.stored())

    assert out.resolved["layouts"] == {
        str(first["id"]): {"x": 0, "y": 10, "w": 6, "h": 10},
        str(second["id"]): {"x": 0, "y": 0, "w": 6, "h": 10},
    }
    assert {operation.card_id for operation in out.operations} == {
        first["id"], second["id"],
    }
    assert any("row 11" in operation.summary for operation in out.operations)
    assert store.get_card(first["id"])["layout"]["y"] == 10
    assert store.get_card(second["id"])["layout"]["y"] == 0
    assert planner.reverse(applied.undo) == "Put the cards back."
    assert store.get_card(first["id"])["layout"]["y"] == 0
    assert store.get_card(second["id"])["layout"]["y"] == 10


def test_auto_pack_new_card_preview_matches_the_frozen_placeholder_layout(
    board, boards,
):
    existing = a_card(board, title="Existing")
    store.update_board(board["id"], layout_mode="auto_pack")
    action = NewCardsAction(say="Adding one.", cards=[CardRequest(
        request_id="r1", question="oil by region", title="New",
        layout={"x": 0, "y": 30, "w": 6, "h": 10},
    )])

    out = planner.resolve(
        "new_cards", action, board=store.get_board(board["id"]),
        boards=store.list_boards(), cards=cards_of(board), client=FakeAsk(),
    )

    assert out.cards[0].layout.y == 10
    assert "row 11" in next(
        operation.summary for operation in out.operations
        if operation.kind == "create_card"
    )
    board_id, placed = planner.create_placeholders(out.stored())
    assert board_id == board["id"]
    assert layout_map(board) == out.resolved["layouts"]
    assert str(placed[0]["card_id"]) in out.resolved["layouts"]
    assert str(existing["id"]) in out.resolved["layouts"]


def test_auto_pack_delete_preview_discloses_every_card_that_will_move(
    board, boards,
):
    cards = [a_card(board, title=f"Card {index}") for index in range(3)]
    store.save_layouts(board["id"], {
        str(card["id"]): {"x": 0, "y": index * 10, "w": 6, "h": 10}
        for index, card in enumerate(cards)
    })
    store.update_board(board["id"], layout_mode="auto_pack")
    out = planner.resolve(
        "delete_card", DeleteCardAction(card_ids=[cards[0]["id"]]),
        board=store.get_board(board["id"]), boards=store.list_boards(),
        cards=cards_of(board), client=FakeAsk(),
    )

    assert {operation.kind for operation in out.operations} == {
        "delete_card", "move_card",
    }
    assert {operation.card_id for operation in out.operations
            if operation.kind == "move_card"} == {
...
def test_a_build_reports_when_its_batch_save_loses_the_card(board, monkeypatch):
    card = store.create_card(board["id"], auto_size_pending=True)
    monkeypatch.setattr(store, "save_rendered_batch_card", lambda *a, **k: None)

    reason = planner.build_card(
        card["id"], "oil by region", chart_hint=None,
        client=FakeAsk(answer()), batch_card_ids=[card["id"]],
        insertion_row=0,
    )

    assert reason == "That card was removed while it was being built."
