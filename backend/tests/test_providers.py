...
def test_openai_parse_validation_is_a_retryable_schema_error():
    """The OpenAI SDK validates inside ``parse``. That failure must enter
    the same corrective retry path as client-side validation failures."""
    with pytest.raises(ValidationError) as invalid:
        AskResponse.model_validate({
            **GOOD,
            "semantic_query": {**GOOD["semantic_query"], "having": "raw"},
        })
    ...
    with pytest.raises(LLMSchemaError, match="having") as caught:
