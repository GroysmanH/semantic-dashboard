...
def test_a_small_multi_series_trend_has_a_shared_non_aggregating_tooltip(lyr):
    ...
    assert [item.get("title") for item in rule["encoding"]["tooltip"]] == [
        "intervention date", "net gain — North", "net gain — South"]
...
def test_too_many_line_series_are_refused_instead_of_becoming_spaghetti(lyr):
    ...
    assert "35 series" in result.error
...
def test_too_many_ranked_categories_are_truncated_and_said_so(lyr):
    """A ranking with more bars than fit is not an unanswerable question.
    Refusing left the person with a blank card and the numbers sitting in
    the SQL panel underneath it; the honest shape is the top of the
    ranking, with the truncation stated.

    Dropped rather than collapsed into "Other": a bar labelled Other on a
    ranked chart reads as a category that outranks real ones."""
    rows = [{"region": f"Region {i:02}", "net_gain": i} for i in range(40)]
    r, _ = spec_for(lyr, rows=rows, dimensions=[DimensionRef(field="region")])

    assert r.chart_type == "bar"
    assert r.chart_rows is not None and len(r.chart_rows) == 30
    assert "top 30 of 40" in r.note
    # The largest, not the first thirty the warehouse happened to return.
    assert {row["net_gain"] for row in r.chart_rows} == set(range(10, 40))
    assert "Other" not in {row["region"] for row in r.chart_rows}


def test_a_ranking_that_fits_is_left_alone(lyr):
    rows = [{"region": f"Region {i:02}", "net_gain": i} for i in range(5)]
    r, _ = spec_for(lyr, rows=rows, dimensions=[DimensionRef(field="region")])

    assert r.chart_rows is None and r.note == ""
...
def test_two_nominal_comparisons_refuse_too_many_categories(lyr, hint):
    ...
    assert result.chart_type == "unplottable"
    assert "35 categories" in result.error
