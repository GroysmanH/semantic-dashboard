...
def test_action_item_transitions_cannot_overwrite_a_cancelled_item(thread):
    plan = pending(thread["id"], "guarded item")
    chat.transition_plan(plan["id"], expected="pending", status="confirmed")
    action = chat.create_action(
        plan, provider="gemini", model="test", effects={},
    )
    item = chat.append_action_item(
        action["id"], ordinal=0, request={"title": "guarded"},
    )
    chat.transition_action_item(
        item["id"], expected="queued", status="cancelled",
    )

    with pytest.raises(chat.ActionItemTransitionError):
        chat.transition_action_item(
            item["id"], expected="running", status="succeeded",
        )

    assert chat.list_action_items(action["id"])[0]["status"] == "cancelled"
...
def test_an_expired_result_is_removed_rather_than_only_marked():
    """The row holds warehouse values. An expiry that greys out the card
    and leaves the data in the table is a promise the app is not keeping,
    and nothing else in this system is going to come along and sweep it."""
    thread = chat.create_thread()
    stale = chat.save_transient(thread["id"], query={"entity": "production"},
                                chart_hint=None, title="old", cache={},
                                ttl_seconds=1)
    with app_pool.connection() as conn, conn.cursor() as cur:
        cur.execute("UPDATE app.chat_transient_result "
                    "SET expires_at = now() - interval '1 minute' "
                    "WHERE id = %s", (stale["id"],))

    # Writing the next result is what pays for the sweep.
    chat.save_transient(thread["id"], query={"entity": "production"},
                        chart_hint=None, title="new", cache={},
                        ttl_seconds=900)

    assert chat.get_transient(stale["id"]) is None


def test_opening_a_conversation_sweeps_what_expired_while_nobody_looked():
    thread = chat.create_thread()
    stale = chat.save_transient(thread["id"], query={"entity": "production"},
                                chart_hint=None, title="old", cache={},
                                ttl_seconds=1)
