...
def test_rank_places_all_null_aggregate_groups_after_ranked_values(
    layer, warehouse_conn,
):
    """A descending PostgreSQL window defaults nulls first unless the
    compiler says otherwise, assigning an all-null group rank 1."""
    query = SemanticQuery.model_validate({
        "entity": "well_interventions",
        "measures": [
            "net_gain",
            {"name": "net_gain", "transform": "rank"},
        ],
        "dimensions": [
            {"field": "intervention_date", "grain": "day"},
            {"field": "contractor"},
        ],
        "limit": 10_000,
    })
    ...
    by_day: dict[object, list[tuple[object, object]]] = {}
    for day, _, value, rank in rows:
        by_day.setdefault(day, []).append((value, rank))
    mixed = [values for values in by_day.values()
             if any(value is None for value, _ in values)
             and any(value is not None for value, _ in values)]

    assert mixed, "the deterministic seed must retain a mixed all-null group"
    for values in mixed:
        null_ranks = [rank for value, rank in values if value is None]
        valued_ranks = [rank for value, rank in values if value is not None]
        assert min(null_ranks) > max(valued_ranks)
