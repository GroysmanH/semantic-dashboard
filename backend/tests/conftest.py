...
def no_leftover_boards(pools):
    ...
    This deletes by "appeared during the run", which cannot tell a board a
    test made from one a person made. That was a real hazard while the
    suite shared a database with the browser -- `make test` could remove a
    dashboard somebody was in the middle of building. The isolation now
    comes from the database (`scripts/ensure_test_db.py`), not from this
    fixture being clever, and this stays only to keep that database tidy.
    """
