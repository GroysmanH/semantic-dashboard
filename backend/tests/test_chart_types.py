...
def test_pie_is_refused_when_the_result_was_truncated(layer):
    ...
    What it falls through to is a bar, and that bar now draws the top of
    the ranking rather than refusing as well. Two guards in a row both
    saying no left the person with a blank card while the numbers sat in
    the SQL panel underneath it."""
    r = drawn(layer, "pie", measures=["oil"],
              dimensions=[{"field": "well_name"}], limit=100)
    assert r.hint_rejected
    assert r.state == "ready"
    assert r.chart_type == "bar"
    # Both facts are stated: why not a pie, and what the bar leaves off.
    assert "pie was asked for" in (r.vega_spec or {}).get("description", "")
    assert "top 30 of" in r.restatement
