...
def test_layout_is_scoped_to_the_board_that_owns_the_card(client):
    ...
    moved = client.patch(f"/boards/{a['id']}/layout", json={
        ...
        "expected_revision": client.get(f"/boards/{a['id']}").json()["revision"],
    })
...
def test_revisions_track_substantive_changes_but_not_render_cache(client):
    ...
    client.patch(
        ...
        json={
            "layouts": {card["id"]: {"x": 6, "y": 2, "w": 6, "h": 10}},
            "expected_revision": after_semantic,
        },
    )
...
def test_duplicating_a_board_copies_layout_policy_without_resizing_old_cards(client):
    board = client.post("/boards", json={"title": "Packed"}).json()
    card = client.post(f"/boards/{board['id']}/cards").json()
    client.patch(f"/boards/{board['id']}", json={"layout_mode": "auto_pack"})

    copy = client.post(f"/boards/{board['id']}/duplicate").json()
    copied_card = client.get(f"/boards/{copy['id']}").json()["cards"][0]

    assert copy["layout_mode"] == "auto_pack"
    assert copied_card["layout"] == card["layout"]
    assert copied_card["auto_size_pending"] is False


def test_duplicating_an_auto_packed_board_preserves_each_cards_layout(client):
    board = client.post("/boards", json={"title": "Packed order"}).json()
    earlier = client.post(f"/boards/{board['id']}/cards").json()
    later = client.post(f"/boards/{board['id']}/cards").json()
    store.update_card(uuid.UUID(earlier["id"]), title="Earlier")
    store.update_card(uuid.UUID(later["id"]), title="Later")
    store.save_layouts(uuid.UUID(board["id"]), {
        earlier["id"]: {"x": 0, "y": 4, "w": 6, "h": 4},
        later["id"]: {"x": 0, "y": 0, "w": 6, "h": 4},
    })
    client.patch(f"/boards/{board['id']}", json={"layout_mode": "auto_pack"})

    with patch("app.store.cards.uuid.uuid4", side_effect=[
    ...
    source_layouts = {
        card["title"]: card["layout"]
        for card in client.get(f"/boards/{board['id']}").json()["cards"]
    }
    copied_layouts = {
        card["title"]: card["layout"]
        for card in client.get(f"/boards/{copy['id']}").json()["cards"]
    }
    assert copied_layouts == source_layouts
